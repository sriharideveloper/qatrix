/* Mutable Three.js uniforms intentionally live outside React state. */
...
float pattern(vec2 p) {
  vec2 movingPoint = p - time * waveSpeed;
  return fbm(p + fbm(movingPoint));
}
...
void main() {
  ...
  const vec3 cream = vec3(0.973, 0.957, 0.910);
  float wave = smoothstep(0.08, 1.18, field);
  vec3 color = mix(cream, waveColor, wave);
...
vec3 dither(vec2 uv, vec3 color) {
  ...
  vec3 adjusted = clamp(color + threshold / levels, 0.0, 1.0);
  vec3 quantized = floor(adjusted * levels + 0.5) / levels;
  return mix(color, quantized, 0.58);
}
...
class RetroEffectImpl extends Effect {
  ...
  set colorNum(value) {
    this.uniforms.get('colorNum').value = Math.max(2, value);
  }
  ...
  set pixelSize(value) {
    this.uniforms.get('pixelSize').value = Math.max(1, value);
  }
...
function DitheredWaves({
  ...
  if (!waveUniformsRef.current) waveUniformsRef.current = {
  ...
  useEffect(() => {
    ...
    waveUniforms.resolution.value.set(
